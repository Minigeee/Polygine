...
class Type<Ret(T::*)(Args...)> : public Type<Ret(T*, Args...)>
{
public:
    Type(int a) : Type<Ret(T*, Args...)>(a) { }

    int m_b;
};

template <typename Ret, typename... Args>
class Type<std::function<Ret(Args...)>> : public Type<Ret(Args...)>
{
public:
    Type(int a) : Type<Ret(Args...)>(a) { }

    int m_c;
};

template <typename F>
Type<typename std::conditional<std::is_member_function_pointer<F>::value, typename F, typename std::remove_reference<F>::type>::type> addTask(F&& func)
{
...
int main()
{
    ...
    std::function<float(float, float)> func(lambda);

    Test a(2);
    Task<float> task = Scheduler::addTask(func, 5.0f, 3.0f);
    Scheduler::finish();

    float result = task.getResult();
