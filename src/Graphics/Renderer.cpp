...
void Renderer::render(Camera& camera, FrameBuffer& target, RenderPass pass)
{
	...
	else
	{
		...
		if (it == m_gBuffers.end())
		{
			...
			buffer->attachColor(Pool<Texture>::alloc(), PixelFormat::Rgba, GLType::Uint8);		//!< Albedo + Ambient factor
		...
		else if (it.value()->getWidth() != target.getWidth() || it.value()->getHeight() != target.getHeight())
		{
			...
			buffer->attachColor(textures[1], PixelFormat::Rgba, GLType::Uint8);		//!< Albedo + Ambient factor
	...
	shader.setUniform("u_albedoAmbient", *buffer->getColorTexture(1));
